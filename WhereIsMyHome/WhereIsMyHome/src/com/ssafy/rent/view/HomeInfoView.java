...
public class HomeInfoView{
	...
	private void showHomeInfo(int code) {
		...
		homeInfoL[0].setText("");
//		homeInfoL[].setText("");
		homeInfoL[1].setText(curHome.getAptName());
		homeInfoL[2].setText(""+curHome.getDealAmount());
		String rent = curHome.getRentMoney();
		if(rent == null) {
			homeInfoL[3].setText("없음");
		}else {
			homeInfoL[3].setText(curHome.getRentMoney());
		}
		homeInfoL[4].setText(""+curHome.getBuildYear());
		homeInfoL[5].setText(curHome.getArea()+"");
		homeInfoL[6].setText(String.format("%d년 %d월 %d일"
											,curHome.getDealYear()
											,curHome.getDealMonth()
											,curHome.getDealDay()
											));
		homeInfoL[7].setText(curHome.getDong());
		homeInfoL[8].setText(curHome.getJibun());
		
		// 환경 추가
		homeInfoL[9].setText(String.format("%d개", curHome.getCleanCnt()));
...
//		System.out.println("###############" + curHome.getImg());
	...
	public void setMain(){
		...
		// 환경시설 개수 추가
		String[] info= {"","주택명","거래금액","월세금액","건축연도","전용면적","거래일","법정동","지번", "환경시설 개수"};
